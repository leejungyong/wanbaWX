...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	foreach ($_FILES as $name => $file) {
      ...
      if($n){
        $data=array('pic'=>'https://'.$server.'/wanba/api/'.$fp,'openid'=>$openid,'orgcode'=>$orgCode,'company'=>$company,'corporate'=>$corporate,'tel'=>$tel,'city'=>$city,'address'=>$address,'status'=>0);
          DB::update("wanba_agent",$data,"id=$n[id]"); 
          $result=array('status'=>true,'msg'=>'操作成功','data'=>$data);
      }else{
        $data=array('pic'=>'https://'.$server.'/wanba/api/'.$fp,'openid'=>$openid,'orgcode'=>$orgCode,'company'=>$company,'corporate'=>$corporate,'tel'=>$tel,'city'=>$city,'address'=>$address);
