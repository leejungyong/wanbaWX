...
switch ($act) {
    ...
    case 'indexData':
        ...
        $hot = DB::fetch_all("SELECT sharepic pic, aid,title FROM wf_wanba_act WHERE template =1 ORDER BY aid DESC  LIMIT 0 , 3");
    ...
    case 'newAct':
        ...
        $cat = $actdata['cat'];
        $teamnum = $actdata['teamNum'];
        $data = array('creator' => $openid, 'title' => $actdata['title'], 'date' => strtotime($actdata['date']), 'teamnum' => $teamnum, 'cat' => $actdata['cat'], 'teamThemeId' =>1);
        $id = DB::insert('wanba_act', $data, 'aid');
        $teamsetting = DB::fetch_all("SELECT `id`, `aid`, `displayorder`, `name`, `desc`, `pic`, `color`  from wf_wanba_team_setting where aid=1 order by displayorder");
        if ($id) {
            for ($i = 0; $i < $teamnum; $i++) {
    ...
    case 'importTemplate':
        ...
        if ($server == "www.wondfun.com") {
            ...
            $list = DB::fetch_all("SELECT `aid`, `title`,route_desc memo,concat('" . $url . "',sharepic) sharepic FROM `wf_wanba_act` " . $where . $order . $limit);
