...
if ($server == "www.wondfun.com") {
    ...
    $swiper[0]['pic'] = 'https://img.wondfun.com/wanba/img/wx1.jpg';
    // $swiper[0]['url'] = 'https://mp.weixin.qq.com/s/964pQtReuhyDPKndQlhH1A';
    $swiper[0]['url'] = 'https://mp.weixin.qq.com/s/Yb04Ew3IZClJNceH-wQE_Q';
    $swiper[1]['pic'] = 'https://img.wondfun.com/wanba/img/wx2.jpg';
    // $swiper[1]['url'] = 'https://mp.weixin.qq.com/s/IA7ScpcOxP6EDJx4iAGXPA';
    $swiper[1]['url'] = 'https://mp.weixin.qq.com/s/dufnnuYup0k6XB0bJOd-tw';
} else {
...
switch ($act) {
        //查询今日进行的活动
    case 'getTodayAct':
        $sql = "SELECT a.title  FROM  `wf_wanba_logs` l,  `wf_wanba_act` a WHERE a.mode =2 AND a.status >=0  AND a.aid = l.aid  AND l.date >= UNIX_TIMESTAMP( DATE_FORMAT( NOW( ) ,  '%Y-%m-%d' ) )  AND l.date <= UNIX_TIMESTAMP( DATE_FORMAT( NOW( ) ,  '%Y-%m-%d 23:59:59' ) )   GROUP BY a.title";
        break;
    case 'getSwiper':
    ...
    case 'indexData':
        ...
        $hot = DB::fetch_all("SELECT sharepic pic, aid,title FROM wf_wanba_act WHERE template =1 ORDER BY aid DESC  LIMIT 0 , 10");
    ...
    case 'actBaseInfo':
        $aid = $arr['aid'];
        $openid = $arr['openid'];
        $data = DB::fetch_first("select `aid`, `title`, from_unixtime(date, '%Y-%m-%d') date,  `sharepic`, `teamnum`,logopic,slogan,cat from wf_wanba_act where aid=$aid and creator='" . $openid . "'");
    ...
    case 'postSetting':
        ...
        $data = array('minenum' => $mineNum, 'minevalue' => $mineMoney, 'gpsEnabled' => $gps, 'offset' => $offset, 'endTime' => $end);
        DB::update("wanba_act", $data, "aid=$aid");
    ...
    case 'getMyQuestionList':
        $openid = $arr['openid'];
        $keyword=$arr['keyword'];
        $where=" where sys=0 and md5(creator)= '" . $openid . "'";
        if ($keyword <> '') {
            $where .= " and  memo like  '%" . $keyword . "%'";
        }
        $pagesize = ($arr['pagesize']) ? $arr['pagesize'] : 10;

        $pageindex = ($arr['currentpage']) ? $arr['currentpage'] * $pagesize : 0;
        $limit = " limit " . $pageindex . "," . $pagesize;
        $list = DB::fetch_all("SELECT `questionid`, `memo`, `qtype`, `answer`, `creator`, `lastpost`, `cat`, `sys`, `url`, `media`, `tag` FROM `wf_wanba_question` ".$where." order by questionid desc " . $limit);
        foreach ($list as $k => $v) {
            
                $list[$k]['pics'] = DB::fetch_all("select * from wf_wanba_question_pic where questionid=$v[questionid]");
            
        }
        $c = DB::fetch_first("SELECT count(questionid) c FROM `wf_wanba_question`".$where);
        $result = array('list' => $list, 'total' => $c['c']);
        echo json_encode($result);
        break;
    case 'myQuestionData':
        $openid = $arr['openid'];
    ...
    case 'pcNewAct':
        ...
        $creator = DB::fetch_first("select openid from wf_wanba_user where md5(openid)='" . $openid . "'");
        if ($creator) {
            if ($creator['openid']) {
                $tasks = $arr['list'];
                $title = $arr['actName'];
                $date = $arr['actDate'] / 1000;
                $openid = $creator['openid'];
                $data = array('creator' => $openid, 'title' => $title, 'date' => $date, 'teamnum' => 6, 'cat' => 0);
                $id = DB::insert('wanba_act', $data, 'aid');
                //队伍设置
                $teamsetting = DB::fetch_all("SELECT `id`, `aid`, `displayorder`, `name`, `desc`, `pic`, `color`  from wf_wanba_team_setting where aid=1 order by displayorder");
                if ($id) {
                    ...
                    foreach ($tasks as $k => $v) {
                        $dt = array('name' => $v['name'],  'latlng' => $v['poi'], 'aid' => $id, 'displayorder' => $k + 1);
                        DB::insert('wanba_task', $dt);
                    }
                    $result = array('status' => true, 'msg' => '创建成功');
                } else {
                    $result = array('status' => false, 'msg' => '创建活动时候队伍设置失败');
                }
            } else {
                $result = array('status' => false, 'msg' => '用户鉴权失败，请重新登录');
            }
        } else {
            $result = array('status' => false, 'msg'  => '用户鉴权失败，请重新登录');
        }
    ...
    case 'newAct':
        ...
        $cat = $actdata['cat'];
        $teamnum = $actdata['teamNum'];
        $data = array('creator' => $openid, 'title' => $actdata['title'], 'date' => strtotime($actdata['date']), 'teamnum' => $teamnum, 'cat' => $actdata['cat']);
        $id = DB::insert('wanba_act', $data, 'aid');
        $teamsetting = DB::fetch_all("SELECT `id`, `aid`, `displayorder`, `name`, `desc`, `pic`, `color`  from wf_wanba_team_setting where aid=1 order by displayorder");
        if ($id) {
            for ($i = 0; $i < $teamnum; $i++) {
            ...
            $point = $arr['point'];
            $tradeno = $arr['tradeno'];
            if ($fromid && $point && $tradeno) {
                DB::query("update wf_wanba_act set isshared=1 where aid=$id");
                DB::query("update wf_wanba_user set point=point+$point where openid='" . $openid . "'");
    ...
    case 'editAct':
        ...
        $data = array('title' => $actdata['title'], 'date' => strtotime($actdata['date']), 'teamnum' => $teamnum, 'sharepic' => $sharepic, 'logopic' => $piclogo, 'slogan' => $slogan, 'cat' => $actdata['cat']);
        //更新活动
        DB::update('wanba_act', $data, "aid=$aid");
    ...
    case 'replaceWithSysTasks':
        ...
        foreach ($sourcetasks as $ke => $va) {
            $sourcetasks[$ke]['pics'] = DB::fetch_all("select url from wf_wanba_pic where taskid=$va[taskid]");
        }
        ...
        $newtasks = DB::fetch_all("select * from wf_wanba_task where aid=$aid order by displayorder");
        foreach ($newtasks as $k => $v) {
            $newtasks[$k]['pics'] = DB::fetch_all("select url from wf_wanba_pic where taskid=$v[taskid]");
        }
    ...
    case 'batchImportPos':
        ...
        if ($mode < 2) {
            ...
            $result = array('status' => false, 'msg' => '活动已正式开始，不允许进行修改');
        }
        echo json_encode($result);
        break;
    case 'getTaskList':
    ...
    case 'getRouteList':
        ...
        $sql = "select count(aid) from  wf_wanba_act " . $where;
        $total = DB::result_first($sql);
        $list = DB::fetch_all("SELECT aid, title,route_desc,city,tag,price from wf_wanba_act " . $where . " order by aid desc   limit " . $pageindex . "," . $pagesize);
        foreach ($list as $k => $v) {
            $list[$k]['route_desc'] = unserialize($v['route_desc']);
            $list[$k]['price'] = strval($v['price']);
    ...
    case 'editRouteList':
        ...
        $tag = $arr['tag'];
        $price = $arr['price'];
        $u = array("title" => $title, 'route_desc' => $route_desc, 'city' => $city, 'tag' => $tag, 'price' => $price);
        $n = DB::update('wanba_act', $u, "aid=$aid and template=1");
        if ($n > 0) {
            $result = array('status' => true, 'msg' => '操作成功', 'data' => $u);
        } else {
    ...
    case 'getSysQuestionList':
        ...
        $total = DB::result_first($sql);
        $syslist = DB::fetch_all("SELECT `questionid`, `memo`, `qtype`, `answer`, `creator`, `lastpost`, `cat`, `sys`, `url`, `media`, `tag` FROM `wf_wanba_question` " . $where . " order by questionid desc  limit " . $pageindex . "," . $pagesize);
        foreach ($syslist as $k => $v) {
            $syslist[$k]['pics'] = DB::fetch_all("select * from wf_wanba_question_pic where questionid=$v[questionid]");
        }
    ...
    case 'getMyWithdrawApplyList':
        $openid = $arr['openid'];
        $list = DB::fetch_all("select from_unixtime(date, '%Y.%m.%d %T') date,cash,status from wf_wanba_withdrawapply where openid='" . $openid . "' order by date desc");
        echo json_encode($list);
        break;
    ...
    case 'getAllMyRouteMonthBill':
        ...
        $month_start = mktime(0, 0, 0, $month, 1, $year); //指定月份月初时间戳  
        $month_end = strtotime(date('Y-m-t 23:59:59', strtotime($m)));
        // $items=DB::fetch_all("select aid from wf_wanba_act where template=1 and creator='".$openid."'");
        // if($items){
        // $aids=array();
        ...
        // }
        // $aid=implode(',',$aids);
        $list = DB::fetch_all("select t.title title, from_unixtime(a.date, '%Y.%m.%d') date,u.nick nick  from `wf_wanba_act` a,`wf_wanba_user` u,`wf_wanba_act` t  where t.template=1 and t.creator='" . $openid . "' and a.bindtemplateid=t.aid  and a.creator=u.openid  and a.template=0 and  a.date>=$month_start and a.date<=$month_end");
        // $list=DB::fetch_all("select a.title title, from_unixtime(a.date, '%Y.%m.%d') date,u.nick nick  from `wf_wanba_act` a,`wf_wanba_user` u,`wf_wanba_act` t  where t.creator='".$openid."' and a.bindtemplateid=t.aid  and a.creator=u.openid and  a.bindtemplateid in (".$aid.") and a.template=0 and  a.date>=$month_start and a.date<=$month_end");
        // }else{
        //     $list=array();
        // }
        echo json_encode($list);
    ...
    case 'searchPoi':
        ...
        $result = searchPoi($url);
        echo json_encode($result);
        break;
    case 'searchByLatlng':
        $poi = $arr['keyword'];

        $result = searchByLatlng($poi);
        echo json_encode($result);
        break;
    case 'gpsToTXPoi':
        $gps = $arr['poi'];
        $result = translateTXmap($gps);
        echo json_encode($result);
        break;
    case 'getOrderList':
    $key = $arr['keyword'];
    ...
    $where=" WHERE o.openid = u.openid  AND o.eventid = a.aid ";
    if($eventtype>=0){
    $where .= ($key == '') ? "  " : "  and  (u.nick like '%" . $key . "%' or a.city like '%" . $key . "%' or o.orderno='".$key."' or o.eventtype=$eventtype ) ";
    }else{
        $where .= ($key == '') ? "  " : "  and  (u.nick like '%" . $key . "%' or a.city like '%" . $key . "%' or o.orderno='".$key."') ";
      
    }
    $begindate=$arr['begindate'];
    $enddate=$arr['enddate'];
    if(strlen($enddate)>0 && strlen($begindate)>0){
        $where .=" and  (o.date >=UNIX_TIMESTAMP('".$begindate."') and o.date<=UNIX_TIMESTAMP('".$enddate."'))";
    }elseif(strlen($begindate)>0){
        $where .=" and  (o.date >=UNIX_TIMESTAMP('".$begindate."'))";
    }
    $pagesize = ($arr['pagesize']) ? $arr['pagesize'] : 10;
    $pageindex = ($arr['currentpage']) ? $arr['currentpage'] * $pagesize : 0;
    $column = $arr['column'];
    ...
    $fullorder = str_replace("normal", "", $fullorder);
    $limit = " limit " . $pageindex . "," . $pagesize;
    $c = DB::fetch_first("SELECT COUNT( o.orderno ) c FROM  `wf_wanba_order` o,  `wf_wanba_user` u,  `wf_wanba_act` a " . $where);
    $data = DB::fetch_all("SELECT  o.orderno orderno, (case o.eventtype when 0 then '创建活动' when 1 then '购买线路' when 2 then '购买门派' else '线下促销' end) eventtype,from_unixtime(o.date, '%Y-%m-%d') date,u.nick nick,o.amount amount, o.memo memo,a.aid aid,a.title title,a.city city FROM `wf_wanba_order` o,`wf_wanba_user` u,`wf_wanba_act` a   " . $where . $fullorder . $limit);
    $result = array('list' => $data, 'total' => $c['c']);
    echo json_encode($result);
    break;
    default:
...
function searchByLatlng($gps)
{
    $url = 'https://apis.map.qq.com/ws/geocoder/v1/?location=' . $gps . '&key=TP5BZ-Q4TWW-NMSRE-RR3OO-UXIJK-L2F2Q';
