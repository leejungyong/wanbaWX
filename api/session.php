...
$now = time();
//防止重复提交
if (isset($_SESSION['lastpost_token12'])) {

    $lastpost_token = $now - $_SESSION['lastpost_token12'];
} else {
    $lastpost_token = $now;
}
$_SESSION['lastpost_token12'] = $now;

require '../../source/class/class_core.php';
$discuz = &discuz_core::instance();
$discuz->init();
switch ($lastpost_token){
    default:
    echo $lastpost_token;
}
exit();
...
if ($_SESSION['lastk']) {
    ...
    $lastpost_token = $now - $_SESSION['lastk'];
    echo '1ok';
    $_SESSION['lastk'] = $now;
    echo $lastpost_token;
} else {
...
switch ($act) {
    ...
    case 'postAIBaseSetting':
        ...
        $d = array('ai_duration' => $ai_duration, 'ai_random' => $ai_random);
        DB::update('wanba_act', $d, "aid=$aid");

        break;
    case 'getAIBaseSetting':
        $aid = $arr['aid'];
        $data = DB::fetch_first("select ai_duration,ai_random from wf_wanba_act where aid=$aid");
        echo json_encode($data);
