...
switch ($act) {
    ...
    case 'getAllMyRouteMonthBill':
        ...
        $flag=$arr['flag'];
        if($flag==0){
            $list = DB::fetch_all("select t.title title, from_unixtime(a.date, '%Y.%m.%d') date,u.nick nick  from `wf_wanba_act` a,`wf_wanba_user` u,`wf_wanba_act` t  where t.template=1 and t.creator='" . $openid . "' and a.bindtemplateid=t.aid  and a.creator=u.openid  and a.template=0");
        }else{
        $year = $arr['year'];
        $month = $arr['month'];
        $m = $year . "-" . $month;
