...
Page({
  ...
  async  _syncMypoint(num,order) {
    return await wxRequest(
      app.globalData.config.apiUrl + 'index.php?act=syncMypoint',
      {
        data: {
  ...
  syncMypoint(num,order) {
    console.log(num)
    this._syncMypoint(num,order)
      .then((res) => {
        console.log(res)
      })
      .catch((err) => {
  ...
  random_No(j) {
    ...
    random_no = aid+'_'+new Date().getTime() + random_no
    return random_no;
  },
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (resultCode == 'FAIL') {
          ...
          that.processPay(param, num,order);
        }
  ...
  /* 小程序支付 */
  processPay: function (param, num,order) {
    let that = this
    wx.requestPayment({
